...
Rscript --vanilla Pheno_Step05_SsbIncomeVars.R 

## *****************************************************************************
## USAGE:
# SCRIPT=Pheno_Step05_SsbIncomeVars.sh
# tr -d '\r' < ${SCRIPT} > ${SCRIPT}.tmp && mv ${SCRIPT}.tmp ${SCRIPT}; chmod +x ${SCRIPT}
# sbatch --account=p805 --time=6:00:00 --cpus-per-task=2 --mem-per-cpu=50G --job-name=Pheno_Step05 ${SCRIPT}
## *****************************************************************************
